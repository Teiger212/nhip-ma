...
Language uses the kit locale cookie `NEXT_LOCALE` (no `/en` or `/vi` URL prefix; that is intentional for this authenticated walk). Open the Walk Operator user menu (sidebar footer) and use **Language** / **Ngôn ngữ** with **EN** / **VI** only (`vi`, not `vn`). Inbox copy lives in `packages/i18n/translations/{locale}/saas.json` under `inbox.*`. To open Vietnamese without the switcher, set `NEXT_LOCALE=vi` and refresh.
