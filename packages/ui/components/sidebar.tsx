...
import { cn, mergeTriggerProps } from "../lib";
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
	...
	return (
		<button
			...
			className={cn(
				"inset-y-0 w-4 group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:inset-y-0 sm:flex absolute z-10 hidden -translate-x-1/2 cursor-pointer transition-all ease-linear after:absolute after:left-1/2 after:w-[2px] hover:cursor-pointer hover:after:bg-sidebar-border",
...
type SidebarMenuButtonRenderProps = React.HTMLAttributes<HTMLElement> & {
	ref?: React.RefCallback<HTMLAnchorElement | HTMLButtonElement>;
	"data-slot"?: string;
	"data-sidebar"?: string;
	"data-size"?: string;
	"data-active"?: boolean;
...
function SidebarMenuButton({
	...
	VariantProps<typeof sidebarMenuButtonVariants> & {
		isActive?: boolean;
		tooltip?: string;
		render?: (props: SidebarMenuButtonRenderProps) => React.ReactElement;
	}) {
				...
				<TooltipTrigger
					render={(triggerProps) => {
						const nodeProps = mergeTriggerProps(triggerProps, {
							...shared,
							...props,
						}) as unknown as SidebarMenuButtonRenderProps;
						if (render) {
							return render(nodeProps);
						}

						return (
							<button
								type="button"
								{...(nodeProps as unknown as React.ButtonHTMLAttributes<HTMLButtonElement>)}
							/>
						);
					}}
				/>
...
function SidebarMenuSubButton({
	...
	if (render) {
		return render({ ...shared, ...props, children });
	}
