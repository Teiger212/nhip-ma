...
found the first of them exploitable: the inbox trusted the session's "active
organization", a field a signed-in user can set through the kit's update-user endpoint
to any organization id. The other two: which office a thread belongs to when the same
guest writes to two offices, and which credentials a reply goes out on.

## Decision

- **Nhịp assigns.** An office is created by a platform admin (`user.role === "admin"`)
  in the kit's admin area, and agents join it through the kit's invitation. Public
  sign-up is closed: an account exists because it was invited into an office.
  Operators never create or pick an office. Manager-invites-agents is a later step that
  un-hides the same kit components inside the office; nothing here forecloses it.
- **One operator, one office.** Membership is read from the membership table on every
  ...
  and a misconfiguration to fix, not a case to support. Accepting an invitation while
  already a member of an office is refused. The session's active-organization field is
  never consulted for access.
- **One host.** The office comes from the account, so a subdomain per office would carry
  no tenancy weight. Per-agency domains are a white-label feature for later.
- **One thread per guest per office.** The thread's identity is (office, pipe, guest).
  ...
  matching.
- **Credentials stay in env for the pilot.** A send is refused (`409
pipe_not_configured`) when the thread's number is not the one this deployment's
...
- The seed creates two logins: the agent (`walk@nhip.local`) and the platform admin
  (`admin@nhip.local`), the admin owning the walk office and the agent a member of it.
