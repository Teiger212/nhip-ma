...
and the CRM connection (ADR 0003) belong to an office, not a person. "Your turn" is an
office queue only if any agent can pick up any thread.

## Decision

- The **office** is the unit of tenancy. It owns its pipes, its CRM connection, its
  agents, and its threads.
- Threads are **shared** within the office: any agent in it can open, draft, and approve
  any thread. Nothing is private to an agent.
- The kit's organization (hidden for the walk) is the office. One agency with one office
  is the MVP shape; a multi-office agency comes later, as several tenants under one
  billing account.
...
- `Conversation.ownerUserId` becomes `officeId` (or the organization id). The store's
  "unowned is visible to everyone" fallback goes once every thread carries an office.
- Webhook-created threads take the office that owns the pipe the message arrived on; a
  pipe-to-office mapping replaces the `INBOX_OWNER_USER_ID` env fallback.
