...
- **Reply-only.** Every approved send answers exactly one guest message. The agent can
  send once per inbound; a second approve against the same inbound is refused (409), as
  a second send on a thread is today.
...
- **Your turn** therefore means "there is an unanswered inbound", which is the same as
  "the guest spoke last" and what the queue rule already says.
...
- The funnel's "engaged" and "in conversation" counts fall out of this naturally: an
  approved send per inbound is the event both are built from.
